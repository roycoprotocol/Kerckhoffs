// Makina governance-slot reads (main.md §7): live RPC via viem, not the subgraph. Compares each
// slot against the expected RoycoFactory address.
...
export async function fetchMakinaSlots(chainId: number): Promise<MakinaSlotView[]> {
  ...
  for (const s of cat.makinaSlots) {
    for (const slot of ["riskManager", "riskManagerTimelock"] as const) {
      const expected = (
        slot === "riskManager" ? s.expectedRiskManager : s.expectedRiskManagerTimelock
        ...
        drift:
          actual && actual !== expected
            ? {
                ...
                actual,
                severity: "HIGH",
                note: "Makina governance slot not pointed at RoycoFactory.",
              }
            : undefined,
      });
    }

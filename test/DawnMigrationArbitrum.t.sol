...
/**
 * @title DawnMigrationArbitrumTest
 * @notice Cross-chain smoke test: Dawn migration applies clean on Arbitrum and post-state
 *         matches the canonical configuration. Arbitrum has only one market (sUSDai).
 */
contract DawnMigrationArbitrumTest is RoleBehaviorBase, MigrateDawn {
    ...
    function test_PostState_TrancheBindings_Consistent() public view {
        string[] memory names = marketNames(ARBITRUM);
        for (uint256 i = 0; i < names.length; i++) {
            MarketAddresses memory m = getMarketAddresses(ARBITRUM, names[i]);
